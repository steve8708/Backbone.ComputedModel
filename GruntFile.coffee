...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    coffee:
      compile:
        files:
          'backbone.computedmodel.js': [ 'src/backbone.computedmodel.coffee' ]
    ...
    uglify:
      main:
        files:
          'backbone.computedmodel.min.js': [ 'backbone.computedmodel.js' ]

    karma:
      options:
        browsers: [ 'PhantomJS' ]
        frameworks: [ 'mocha', 'chai' ]
        files: [ 'backbone.computedmodel.js', 'test/**/*.coffee' ]
    ...
    replace:
      ...
      debugJS:
        src: [ 'backbone.computedmodel.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.computedmodel.js v#{ packageVersion() }" +
            " (coffeescript output) */ \n\n"
        ]

      minJS:
        src: [ 'backbone.computedmodel.min.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.computedmodel.min.js v#{ packageVersion() } */ \n"
        ]
    ...
    bump:
      options:
        ...
        commitFiles: [
          'package.json'
          'src/backbone.computedmodel.coffee'
          'backbone.computedmodel.js'
          'backbone.computedmodel.min.js'
        ]
        pushTo: 'origin'
